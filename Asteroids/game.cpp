...
void Game::handleCollisions() {
   ...
   for (Rock * rock : rocks) {
      if (handleCollision(ship, rock)) return;
      if (handleCollisions(lasers, rock)) return;
   }
}

bool Game::handleCollisions(std::vector<Laser> & lasers, Rock * rock) {
    // for each laser
    for (Laser & laser : lasers)
        if (handleCollision(laser, rock))
           return true;

    return false;
}

bool Game::handleCollision(Ship & ship, Rock * rock) {
    if (rock == NULL || !checkCollision(&ship, rock))
        return false;
...
bool Game::handleCollision(Laser & laser, Rock * rock)
{
   if (rock == NULL || !checkCollision(&laser, rock)) return false;

   // collision
   splitRock(rock, laser);
...
void Game::splitRock(Rock * rock, MovingObject & obj) {
    ...
    newRock1 = createRock(newRockIndex, rock->getPoint(), rock->getVelocity() + offsetV1, false);
    newRock2 = createRock(newRockIndex, rock->getPoint(), rock->getVelocity() + offsetV2, false);
